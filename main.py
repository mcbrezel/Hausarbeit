...
if __name__ == "__main__":
    ...
    df_sel_ideal = pd.DataFrame(data=np.hstack((np.atleast_2d(df_ideal["x"]).T, selected_ideal_funcs)), columns=np.hstack(("x", selected_ideal_funcs.columns)))
    train_deltas = np.full(shape=df_train.iloc[:, 1:].shape, fill_value=sys.float_info.max)
    df_train_deltas = pd.DataFrame(data=train_deltas, index=df_train["x"], columns=selected_ideal_funcs.columns, dtype="float32")
    df_test_deltas = pd.DataFrame(data=np.full(shape=(count_xs_test, count_ys_sel_ideal), fill_value=-1), index=df_test["x"], columns=df_sel_ideal.columns[1:], dtype="float32")
    ...
    for x_test in df_test["x"]:
        for test_index in np.where(df_test["x"] == x_test)[0]:
            df_test_deltas.iloc[test_index] = (df_sel_ideal[df_sel_ideal["x"] == x_test].iloc[:, 1:] - df_test.loc[df_test["x"] == x_test]["y"][test_index]) ** 2
            col_indices_fittable = np.where((df_train_deltas.loc[x_test] * math.sqrt(2)) - df_test_deltas.loc[x_test] > 0)
            df_fittable_deltas = df_test_deltas.iloc[test_index].iloc[col_indices_fittable[0]]
            col_index_fittable_min_delta = np.where(df_test_deltas.iloc[test_index] == np.min(df_test_deltas.iloc[test_index]))
            fittings.append(Fitting(x=x_test, y=df_test["y"].iloc[test_index], delta=df_test_deltas.iloc[test_index, col_index_fittable_min_delta[0][0]], ideal_function=df_test_deltas.columns[col_index_fittable_min_delta[0][0]]))
